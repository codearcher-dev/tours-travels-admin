...
export default function Header({ setIsOpen }) {
  ...
  return (
    <header className="flex h-16 flex-shrink-0 items-center justify-between border-b border-gray-200 bg-white px-4 sm:px-6 lg:px-8 relative z-30">
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <>
              ...
              <div className="absolute right-0 z-50 mt-2 w-80 origin-top-right rounded-xl bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none animate-in fade-in zoom-in-95 duration-100">
                  ...
                  {notifications.length > 0 ? (
                    <div className="divide-y divide-gray-100">
                      {notifications.map((notif) => (
                        <div key={notif.id} className={`p-4 hover:bg-gray-50 transition-colors ${!notif.isRead ? 'bg-blue-50/50' : ''}`}>
                          <div className="flex items-start gap-3">
                             <div className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 ${!notif.isRead ? 'bg-primary-500' : 'bg-gray-300'}`}></div>
                ...
                <div className="border-t border-gray-100 p-2 bg-gray-50/50 rounded-b-xl text-center">
                   <Link to="/enquiries" onClick={() => setShowNotifications(false)} className="text-xs font-semibold text-primary-600 hover:text-primary-700">
                     View all enquiries
                   </Link>
                </div>
              </div>
